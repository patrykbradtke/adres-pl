 ...
 *
 *   node --experimental-strip-types packages/etl/test/bench-real.ts [sciezka]
 */
import { readFileSync } from 'node:fs';
import { SearchIndex } from '../../api/src/search/engine.ts';

const path = process.argv[2] ?? './data/index/current.bin';
const memBefore = process.memoryUsage().rss;
const buf = readFileSync(path);
const idx = new SearchIndex(buf);
const rss = (process.memoryUsage().rss - memBefore) / 1048576;

console.log(`artefakt:   ${path}  ${(buf.length / 1048576).toFixed(1)} MB`);
...
const all: number[] = [];
for (const [q, description] of QUERIES) {
  ...
  console.log(
    `  ${q.padEnd(14)} ${description.padEnd(26)} p50 ${times[Math.floor(ITER * 0.5)].toFixed(2).padStart(7)} ms` +
...
for (const q of ['warszawa', 'kosciuszki warszawa', 'mickievicza', 'nowa wies']) {
  ...
  for (const s of idx.search(q, { limit: 3 })) {
    console.log(`      ${String(s.score).padStart(5)}  ${s.label}   [${s.type}, pkt=${s.addressPointCount}]`);
  }
}
